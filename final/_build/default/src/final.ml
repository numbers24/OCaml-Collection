...
(*
𝜃{X/Y} : Substitution.map (fun t-> sub {X/Y} into t) 
...
match t1,t2 with
| Variable x, _ -> (*X dne in Y -> 𝜃{X/Y} ∪ {X/Y}*)
if t1=t2 then s
...
| _,Variable x -> (*Y dne in X -> 𝜃{Y/X} ∪ {Y/X}*)
if t1=t2 then s
...
|Function(h1,b1),Function(h2,b2)-> (*X is f(X1,...,Xn) and Y is f(Y1,...,Yn) -> (fold_left (fun 𝜃 (X,Y) -> unify(X,Y,𝜃)) 𝜃 [(X1,Y1),...,(Xn,Yn)]) *)
